...
| Superadmin | rol técnico (código/infra/base de datos), login propio, MFA (TOTP) disponible — **configurable on/off por el propio superadmin desde Configuración, no obligatorio sin excepción** (decisión revisada 2026-07-15, ver nota abajo), distinto de Admin_prestadora y de Admin_plataforma. Acceso de Panel únicamente a una prestadora de prueba fija (sandbox) — vedado el acceso a cualquier prestadora real, ninguna tarea técnica lo justifica. No es cross-tenant, no confundir ni fusionar con Admin_prestadora ni Admin_plataforma en código ni en UI. Diseño completo en `docs/PLAN_MULTITENANT_PLM.md` 3.4/3.4.1 (agregado 2026-07-13, reemplaza el alcance cross-tenant sin límites que este rol tenía documentado antes de esa fecha) |
| Admin_plataforma (rol técnico, `usuarios.rol`, agregado 2026-07-13) | administrativo de negocio real de toda la plataforma (todas las prestadoras licenciatarias) — comercial, administrativo, gestión de cuentas Admin_prestadora. Login propio, MFA (TOTP) disponible — mismo interruptor on/off que maneja el superadmin, ver nota abajo. Entra a una prestadora real una a la vez, nunca varias a la vez, bajo el "modo dentro de una prestadora" (banner notorio, advertencia agregada a las confirmaciones de acciones destructivas de la Regla 4, log de auditoría de todo login/acción sensible, timeout de 5 min de inactividad + tope absoluto de 60 min con aviso a los 50 si sigue activo) — mientras está adentro, cero visibilidad de cualquier otra prestadora. Nombre elegido deliberadamente desacoplado de la razón social de la empresa dueña del software (no "admin_plm"), para que un cambio societario futuro no deje resabios de nombre en código/RLS. Diseño completo: `docs/PLAN_MULTITENANT_PLM.md` 3.4/3.4.1 |
...
**Nota — MFA de Superadmin/Admin_plataforma pasa de "obligatorio" a "configurable on/off"
(2026-07-15, decisión explícita del Desarrollador, pisando el diseño anterior de
`docs/PLAN_MULTITENANT_PLM.md:416`):** al construir el pendiente #30 ítem H se le señaló al
Desarrollador que un interruptor manejado por el propio superadmin contradice la razón de
ser de "obligatorio" (evitar que una sesión de superadmin comprometida se autoexima de la
protección). Decisión del Desarrollador, con el motivo explícito de no sumar fricción
mientras el sistema todavía se está puliendo y los usuarios se acostumbran: que sea
configurable on/off, manejado únicamente por el superadmin. Queda registrado acá para que
no se reintroduzca "obligatorio sin excepción" como si fuera el diseño vigente en una
sesión futura sin releer esta nota.
